...
export class HttpMoviesService {
  ...
  headers(): Observable<HttpResponse<Movie[]>> {
    //observe: 'response' - informuje że nie chcemy mieć dostępu tylko do body ale także do headers!!
    const myHeaders = new HttpHeaders({
      Authorizations: 'my_token',
      'Content-Type': 'application/json',
      'X-Custom-Header': 'zacznij_programowac'
    });
    // nagłówek z poza specyfikacji zaczynamy od Xa

    return this.http.get<Movie[]>(this.url, {observe: 'response', headers: myHeaders})
      .pipe(
        tap((res)=> {
